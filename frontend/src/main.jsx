...
const apiBase = (import.meta.env.VITE_CODETRACE_API_BASE || '').trim() || '/api'
if (apiKey) {
  ...
  if (!apiBaseUrl || apiBasePath === '/') {
    ...
    window.fetch = (input, init) => {
      ...
      if (crossOrigin) {
        ...
        if (input instanceof Request) {
          // 显式复制原 Request 到改写后的地址（标准 RequestInit 写法）
          const copied = new Request(resolveApiUrl(rawUrl), {
            method: input.method,
          ...
          return originalFetch(copied, { ...reqInit, headers })
        }
      }
