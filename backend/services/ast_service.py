...
def trace_function_across_commits(repo_path: Path, file_path: str, function_name: str) -> list[dict]:
    ...
    for c in commits:
        ...
        if matched:
            ...
            if result and result not in ("UNKNOWN", "DELETED"):
                name_found = result.strip()
            ...
            history.append({
                ...
                "date": c["date"],
                "message": c["message"],
                "function": "deleted" if function else "file_not_found",
                "llm_note": result if result else "函数已消失，LLM 无法定位"
            })
